...
import { Calendar, MapPin, ThumbsDown, ThumbsUp, User } from "lucide-react";
...
const SuggestionCard: React.FC<SuggestionCardProps> = ({ suggestion, className }) => {
  ...
  const upvotePercentage =
    totalVotes > 0 ? (suggestion.votes.up / totalVotes) * 100 : 0;
  ...
  return (
    <div className="card p-6 hover:shadow-lg transition-all duration-200">
      ...
      <div className="flex items-center space-x-1 text-sm text-gray-500 mb-4">
        ...
        <span>
          {suggestion.position.lat.toFixed(4)},{" "}
          {suggestion.position.lng.toFixed(4)}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <button
            ...
            } ${!user ? "cursor-not-allowed opacity-50" : "hover:shadow-sm"}`}
          >
